# MCP (Model Context Protocol) Support for icron

This module adds MCP client support to icron, enabling integration with the growing ecosystem of MCP servers.
  ...
  "tools": {
    "mcp": {
      ...
      "servers": {
        "calculator": {
          "command": "python",
          "args": ["~/.icron/mcp/servers/calculator.py"]
        }
      }
    }
  }
...
Save this as `~/.icron/mcp/servers/calculator.py`:
...
├── MCPToolAdapter     # Adapts MCP tools to icron Tool interface
...
```
WARNING: MCP support not available. Install with: pip install icron-ai[mcp]
```
